...
<html lang="fr">
	<head>
		<title>Page d'offre - Home Exchange Manager</title>
		...
		<script>
			...
			var idMember =  '<%=session.getAttribute("idMember")%>';
			$(function () {
				if (role == 'admin') {
...
<table class="table table-group" id="offerTable">
  ...
  <tr>
  	  <td><%=offerBean.getLocation()%></td>
      <td><%=offerBean.getDate_start()%></td>
      <td><%=offerBean.getDate_end()%></td>
      <td><%=offerBean.getDescription()%></td>
      <td><%=offerBean.getServices()%></td>
      <td><%=offerBean.getLimitations()%></td>
      <td><img src=<%="data:image/png;base64," + offerBean.getOffer_image_string()%>></td>
    </tr>
     </tbody>
</table>
<div id="functions">
